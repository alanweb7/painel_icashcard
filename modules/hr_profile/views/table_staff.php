...
foreach ($custom_fields as $field) {
	$select_as = 'cvalue_' . $field['slug'];
	if ($field['type'] == 'date_picker' || $field['type'] == 'date_picker_time') {
		$select_as = 'date_picker_cvalue_' . $field['slug'];
	}
...
// filtro customizado Nome fantasia
$team_manage = $this->ci->input->post('hr_profile_fantasy_name');
if ($team_manage) {
	// $this->ci->db->like(db_prefix() . 'staff.team_manage', $team_manage);
	$where_team_manage = db_prefix() . 'staff.team_manage = ' . $team_manage;
	array_push($where, 'AND' . $where_team_manage);
}

// FILTRO CUSTOMIZADO BUSCA POR CNPJ
// $('input[name="staff_cnpj"]').on('keyup change', function() {
// 	$('.table-staff').DataTable().ajax.reload();
// });

$staff_cnpj = $this->ci->input->post('staff_cnpj');

if (!empty($staff_cnpj)) {
	// Remove máscara (pontos, traços, etc) se necessário
	// $staff_cnpj = preg_replace('/\D/', '', $staff_cnpj);

	// Faz a busca na tabela de campos personalizados
	$this->ci->db->select('relid');
	$this->ci->db->from(db_prefix() . 'customfieldsvalues');
	$this->ci->db->where('fieldto', 'staff');
	$this->ci->db->where('fieldid', 25);
	$this->ci->db->like('value', $staff_cnpj, 'after'); // 'after' = começa com
	$row = $this->ci->db->get()->row();

	// Se encontrou, aplica o filtro pelo staffid
	if ($row) {
		$where[] = " AND " . db_prefix() . 'staff.staffid = ' . (int)$row->relid;
	} else {
		// Garante que nenhum resultado será retornado
		$where[] = '1=0';
	}
}
...
$where[] = 'AND ' . db_prefix() . 'staff.job_position != (1)';
...
usort($result['rResult'], function ($a, $b) {
	return ($b['staffid'] ?? 0) <=> ($a['staffid'] ?? 0);
});
...
foreach ($rResult as $aRow) {
	...
	foreach ($colunas as $col) {
		...
		if ($col == 'datecreated') {
			...
			if (staff_can('delete', 'user_manager') || is_admin()) {
		...
		} elseif ($col == 'team_manage') {
			if ($aRow['team_manage'] != '') {

				$team =  $this->ci->staff_model->get($aRow['team_manage']);

				$gerente = "{$team->firstname} {$team->lastname} ";
	...
	// 	if (strpos($col, 'as') !== false && !isset($aRow[$aColumns[$i]])) {
