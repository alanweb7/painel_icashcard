<script>
	$(function() {
		...
		var StaffServerParams = {
			...
			'hr_profile_fantasy_name': '[name="hr_profile_fantasy_name"]',
			'staff_cnpj': '[name="staff_cnpj"]',
		};
		...
		$.each(StaffServerParams, function() {
			$('#hr_profile_fantasy_name').on('change', function() {
		...
		// BUSCA POR CNPJ
		$('#staff_cnpj').on('keyup change', function() {
			const cnpj = $(this).val().replace(/\D/g, ''); // remove tudo que não for número

			if (cnpj.length >= 5 || cnpj.length === 0 ) {
				table_staff.DataTable().ajax.reload()
					.columns.adjust()
					.responsive.recalc();
			}
		...
		// mascara de cjpj
		document.getElementById('staff_cnpj').addEventListener('input', function(e) {
