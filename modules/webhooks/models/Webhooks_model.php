...
class Webhooks_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        if (empty($data)) {
            log_message('error', 'Tentativa de adicionar um webhook com dados vazios.');
            return false;
        }
    
        // Tenta inserir os dados na tabela de webhooks
        if ($this->db->insert($this->webhook_table, $data)) {
            $insert_id = $this->db->insert_id();
    
            // Chama os métodos adicionais após o sucesso da inserção
            try {
                // \modules\webhooks\core\Apiinit::the_da_vinci_code('webhooks');
                \modules\webhooks\core\Apiinit::ease_of_mind('webhooks');
            } catch (Exception $e) {
                // Loga qualquer exceção gerada nos métodos adicionais
                log_message('error', 'Erro ao executar hooks após adicionar webhook: ' . $e->getMessage());
            }
    
            return $insert_id;
        }
    
        // Loga o erro em caso de falha na inserção
        log_message('error', 'Erro ao adicionar webhook: ' . $this->db->last_query());
        return false;
    }
