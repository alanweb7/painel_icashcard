        ...
        <div class="row">
            <div class="panel_s">
                <div class="panel-body">
                    <!-- Este trecho foi modificado para exibir todos os meses no resultado de busca por data (período) 
                            para ajuste do meso deve-se ajustar a funcao get_where_report_period($field = 'date') no controller
                    -->
                    <div class="row">
                    ...
                    <div class="row">
                        <?php
                        if (staff_can('view_filters', 'corban_commissions')):
                        ?>
                            <?php echo form_hidden('is_client', 0); ?>
                            <!-- Filtro de data personalizado -->

                            <div class="col-md-5">
                                <label for="custom_date_from"><?php echo "Data de Liberação"; ?></label>
                                <input type="date" id="custom_date_from" name="custom_date_from" class="form-control">
                            </div>
                            <div class="col-md-5">
                                <label for="custom_date_to"><?php echo "Até"; ?></label>
                                <input type="date" id="custom_date_to" name="custom_date_to" class="form-control">
                            </div>
                            <div class="col-md-2" style="padding-top:25px;">
                                <button id="filter_custom_dates" class="btn btn-primary"><?php echo "Filtrar por data"; ?></button>
                            </div>
...
<script>
    $(function() {
        $('#role_filter').on('change', function() {
            var staffIds = $(this).val(); // valores selecionados no primeiro select

            if (!staffIds || staffIds.length === 0) {
                $('#staff_filter').html('').selectpicker('refresh');
                return;
            }

            $.post(admin_url + 'commission/get_clients_by_staff', {
                staff_ids: staffIds
                ...
                console.log(Obj);
                console.log("Staffs: ");
                console.log(Obj.staffs);
                var options = '';

                $.each(staffs, function(i, client) {
                    options += '<option value="' + client.staffid + '">' + client.nome_fantasia + '</option>';
                });

                $('#staff_filter').html(options).selectpicker('refresh');
            });
        });
    });
</script>
