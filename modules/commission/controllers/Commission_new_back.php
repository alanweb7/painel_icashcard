    ...
    public function commission_table()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                //     for ($i = 1; $i <= $limite; $i++) {
                //         $output['aaData'][0][] = ''; // Adiciona colunas vazias para manter o layout
                //     }
                ...
                /** ----- COLUNA PRAZO ------- */
                $_data = rand(1, 10); // ou use mt_rand(1, 10) para melhor desempenho
                $row[] = $_data;

                /** ----- COLUNA VALORES ------- */
                $row[] = app_format_money($aRow['subtotal'], $currency->name);
                $footer_data['total_liq'] += $aRow['subtotal'];
        ...
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                if ($aRow['convert_expense'] == 0) {
                    ...
                    $_data = '<a href="' . admin_url('commission/baixar_pdf/' . $aRow['id']) . '" class="btn btn-success btn-icon">' . _l('view_expense') . '</a>';
                }
                $row[]  = $_data;
