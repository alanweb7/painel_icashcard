...
class Commission extends AdminController
{
    ...
    public function commission_table()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            } elseif ($this->input->post('role_filter')) {
            ...
            if ($this->input->post('is_process_data')) {
                ...
                foreach ($rResult as $aRow) {
                    $output['aaData'][] = [
                        '<script>window.location.href = "../commission/manage_commission_receipt";</script>',
                        '', // Colunas vazias para manter o layout
                        '',
                        '',
                        '',
                        '',
            ...
            foreach ($rResult as $aRow) {
                ...
                // Verifica se o agente mudou
                if ($agente_atual != $staffid) {
                    // Se já existe um agente atual (não na primeira iteração), exibe os ganhos totais

                    // Adiciona a linha com o nome do agente e os ganhos acumulados
                    $output['aaData'][] = [
                        '<div class="td_container"><div class="agente-linha"><strong>Agente: ' . get_staff_full_name($staffid) . '</strong><span class="total-comissao"> Total Ganho: R$ ' . app_format_money($ganhos_por_agente[$staffid], $currency->name) . '</span></div></div>',
                        '', // Colunas vazias para manter o layout
                        '',
                        '',
                        '',
                ...
                // Monta a linha normal da comissão
    ...
    public function generate_commission_data($data)
    {
        ...
        $list_commission = [];
        $result = [];
        $amount = 40.00; // Exemplo de valor para o campo "amount"
        $date = "01-10-2024"; // Exemplo de valor para a data
        ...
        foreach ($data as $commission) {
            ...
            // Condição personalizada: verificando invoice_id, staffid e amount > amount_paid ou associação na tabela commission_receipt_detail
            ...
            foreach ($commissions as $c) {
                $list_commission[] = $c['id']; // Adicione os IDs ao array
            }

            // Crie o array final com todos os dados necessários
            $result = [
        ...
        // Retorne o array final (você pode salvar no banco ou processar como quiser)
        return true;
    }
    ...
    /**
     * get data commission process
     * 
     * @return     json
     */
    public function commission_process_data($data)
    {
    ...
    public function sendNotificationWebhookExemplo($data)
    {
        ...
        $url = 'https://webhook.site/71b55698-e6f5-4ba4-8dda-15c26312115a';
            ...
            foreach ($rResult as $aRow) {
                ...
                if ($aRow['convert_expense'] == 0) {
                    if (has_permission('commission_receipt', '', 'create') || is_admin()) {
                        $_data = '<a href="javascript:void(0)" onclick="convert_expense(' . $aRow['id'] . ',' . $aRow['amount'] . '); return false;" class="btn btn-warning btn-icon">' . _l('commission_paid') . '</a>';
                    } else {
