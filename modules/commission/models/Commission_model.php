...
class Commission_model extends App_Model
{
	...
	public function sendNotificationWebhookExemplo($data)
	{
		...
		$url = 'https://webhook.site/9a498b75-1493-446a-8d83-946156dcf927';

		// Dados fictícios que serão enviados no POST
		// $data = array(
		//     'nome' => 'João Silva',
	...
	public function get_receipt($id = '', $where = [])
	{
		...
		if ($id != '') {
			...
			if ($receipt) {
				...
				$expense_id = $receipt->convert_expense;
				$receipt->expense_details = $this->expenses_model->get($expense_id);
	...
	{
		// Certificar-se de que o ID da fatura é um número inteiro válido
		$invoice_id = intval($invoice_id);
		if ($invoice_id <= 0) {
			return false;
		}

		// Buscar os detalhes da fatura pelo número
		$this->db->select('inv.*, st.firstname as staff_firstname, st.lastname as staff_lastname, st.email as staff_email');
		$this->db->from(db_prefix() . 'invoices as inv');
		$this->db->join(db_prefix() . 'staff as st', 'st.staffid = inv.addedfrom', 'left');
		$this->db->where('inv.id', $invoice_id);
		$invoice = $this->db->get()->row();

		// Se não encontrar a fatura, retornar falso
		if (!$invoice) {
		...
		$proposal_id = $invoice->proposal_id ?? false;

		if ($proposal_id) {
			// Verifique se o ID da proposta é válido
			if (!empty($proposal_id)) {
				// Buscar os detalhes da proposta usando o ID da proposta
				$this->db->select('*');
				$this->db->from(db_prefix() . 'proposals'); // Tabela de propostas
				$this->db->where('id', $proposal_id);
				$proposal = $this->db->get()->row();


				// Se encontrar a proposta
				if ($proposal) {
					// Adicionar os dados da proposta ao objeto fatura
					$invoice->proposal_details = $proposal;

					// ----- Buscar campos personalizados da proposta -----
					$custom_fields = get_custom_fields('proposal'); // Obtém os campos personalizados da proposta
					$proposal_custom_fields = [];

					foreach ($custom_fields as $field) {
						$proposal_custom_fields[$field['name']] = get_custom_field_value($proposal_id, $field['id'], 'proposal');
					}

					// Adicionar campos personalizados ao objeto fatura
					$invoice->proposal_custom_fields = $proposal_custom_fields;

					// ----- Buscar itens da proposta -----
					$this->db->select('*');
					$this->db->from(db_prefix() . 'itemable'); // Tabela que armazena os itens (normalmente 'itemable')
					$this->db->where('rel_id', $proposal_id); // ID da proposta
					$this->db->where('rel_type', 'proposal'); // Tipo de relacionamento 'proposal'
					$proposal_items = $this->db->get()->result();

					// Adicionar itens ao objeto fatura
					$invoice->proposal_items = $proposal_items;
				} else {
					// Caso não encontre a proposta
					$invoice->proposal_details = 'Proposta não encontrada';
				}
			} else {
				$invoice->proposal_details = 'ID da Proposta está vazio';
			}
		} else {
			$invoice->proposal_details = 'Campo Proposta não encontrado nos campos personalizados da fatura';
		}


		// Retornar o objeto completo da fatura
		return $invoice;
	}
		...
		if ($only_id == true) {
			...
			if ($commission_receipt_detail) {
				foreach ($commission_receipt_detail as $key => $value) {
					...
					// Exemplo de preenchimento de um valor estático
					$commission_receipt_detail[$key]['banco'] = "WEB CASH";

					// Obter campos personalizados do cliente
					$client_custom_fields = [];
					$custom_fields = get_custom_fields('customers');
					foreach ($custom_fields as $field) {
						$client_custom_fields[$field['name']] = get_custom_field_value($value['clientid'], $field['id'], 'customers');
					}
					$commission_receipt_detail[$key]['client_custom_fields'] = $client_custom_fields;

					/** ------- ITEMS DA INVOICE -------->>>> */
					// Obter itens da invoice usando a tabela itemable
					$this->db->select('*');
					$this->db->where('rel_id', $value['invoice_id']); // Utilize o invoice_id atual para buscar itens
					$this->db->where('rel_type', 'invoice'); // Especificar que o tipo de item está relacionado a 'invoice'
					$invoice_items = $this->db->get(db_prefix() . 'itemable')->result_array();

					// Adicionar os itens da invoice ao detalhe da comissão
					$commission_receipt_detail[$key]['invoice_items'] = $invoice_items;

					/** <<<<<------- ITEMS DA INVOICE ----------- */

					// Obter campos personalizados do staff
					$staff_custom_fields = [];
					$custom_fields_staff = get_custom_fields('staff');
					foreach ($custom_fields_staff as $field) {
						$staff_custom_fields[$field['name']] = get_custom_field_value($value['staffid'], $field['id'], 'staff');
					}
					$commission_receipt_detail[$key]['staff_custom_fields'] = $staff_custom_fields;
				}
			} else {
				// Retornar erro se não houver resultados
				return ['error' => 'Nenhum detalhe do recibo encontrado'];
			}
		}
	...
	function receipt_custom_pdf($receipt)
	{
		return app_pdf('receipt', module_dir_path(COMMISSION_MODULE_NAME, 'libraries/pdf/Receipt_custom_pdf'), $receipt);
	}
