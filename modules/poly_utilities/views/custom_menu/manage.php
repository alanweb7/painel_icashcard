                                        ...
                                        <div id="poly-active-menu" class="list-group col nested-sortable">
                                            ...
                                            <template v-for="(item, parent_index) in menu_items" :key="item.id">
                                                ...
                                                <div :style="handleDividerStyles(item)" v-if="item.slug && !item.slug.includes('root')" :class="['list-group-item', `nested-${parent_index}`]" :data-id="item.slug" :data-icon="item.svg ? item.svg : item.icon" :data-badge="JSON.stringify(item.badge)" :data-css="item.css" :data-href_original="item.href_original" :data-href="item.href" :data-target="item.target" :data-rel="item.rel" :data-type="item.type" :data-roles="item.roles" :data-users="item.users" :data-is_custom="item.is_custom" :data-name="handleHtmlContent(item.name)" :data-slug="item.slug" :data-parent_slug="item.parent_slug" :data-disabled="item.disabled" :data-level="0" :data-popup_description="item.popup_description">
                                                    ...
                                                    <div v-if="item.children && item.children.length" :class="['tw-mt-2 list-group nested-sortable poly-hide']">
                                                        <template v-for="(item_child, sub_index) in item.children" :key="item_child.id">
                                                            <div :style="handleDividerStyles(item_child)" v-if="item_child.slug && !item_child.slug.includes('_add')" :class="['list-group-item sub',`nested-${sub_index}`]" :data-id="item_child.slug" :data-type="item_child.type" :data-roles="item_child.roles" :data-users="item_child.users" :data-is_custom="item_child.is_custom" :data-name="handleHtmlContent(item_child.name)" :data-href_original="item_child.href_original" :data-css="item_child.css" :data-href="item_child.href" :data-target="item_child.target" :data-rel="item_child.rel" :data-icon="item_child.svg ? item_child.svg : item_child.icon" :data-badge="JSON.stringify(item_child.badge)" :data-slug="item_child.slug" :data-parent_slug="item_child.parent_slug" :data-disabled="item_child.disabled" :data-level="1" :data-popup_description="item_child.popup_description">
                                                                ...
                                                                <!-- Submenu 3 container area -->
                                                                <div v-if="item_child.children && item_child.children.length" :class="['tw-mt-2 list-group sub2 nested-sortable poly-hide']">
                                                                    <template v-for="(item_child2, sub2_index) in item_child.children" :key="item_child2.id">
                                                                        <div :style="handleDividerStyles(item_child2)" v-if="item_child2.slug && !item_child2.slug.includes('_add')" :class="['list-group-item sub sub2',`nested-${sub2_index}`]" :data-id="item_child2.slug" :data-type="item_child2.type" :data-roles="item_child2.roles" :data-users="item_child2.users" :data-is_custom="item_child2.is_custom" :data-name="handleHtmlContent(item_child2.name)" :data-href_original="item_child2.href_original" :data-css="item_child2.css" :data-href="item_child2.href" :data-target="item_child2.target" :data-rel="item_child2.rel" :data-icon="item_child2.svg ? item_child2.svg : item_child2.icon" :data-badge="JSON.stringify(item_child2.badge)" :data-slug="item_child2.slug" :data-parent_slug="item_child2.parent_slug" :data-disabled="item_child2.disabled" :data-level="2" :data-popup_description="item_child2.popup_description">
                                                                            ...
                                                                            <span @click.stop="handleDelete(item_child2)" :data-id="item_child.slug" v-if="item_child2.is_custom=='true'" class="poly-cursor tw-mr-1 text-muted pull-right"><i class="fas fa-trash"></i></span>
                                                                            <span @click.stop="handleEdit(item_child2)" v-if="item_child2.is_custom=='true'" class="poly-cursor poly-menu-item-edit tw-mr-1 text-muted pull-right"><i class="fas fa-pencil"></i></span>
                                                                ...
                                                                <!-- END: Submenu 3 container area -->
                                                                <!-- Empty submenu 3 container area -->
                                                                <div v-if="(!item_child.type || item_child.type !== 'divider') && (item_child?.is_custom ? item_child.is_custom !== 'false' : 'true')" class="tw-mt-2 list-group sub3 nested-sortable">
                                                                    <div :class="['list-group-item sub empty', `nested-${sub_index}`]"></div>
                                                                </div>
                                                                <!-- END: Empty submenu 3 container area -->
                                                    ...
                                                    <div v-if="(!item.type || item.type !== 'divider') || (item?.is_custom ? item.is_custom !== 'false' : 'true')" class="tw-mt-2 list-group nested-sortable">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div id="poly-custom-menu" class="list-group col">
                                            ...
                                            <div v-for="item in custom_menu_items" class="list-group-item">
                                                ...
                                                <div><i class="fa-solid fa-list fa-fw"></i> Type: {{item.type}}<span @click.stop="handleDelete(item)" :data-id="item.slug" v-if="item.is_custom=='true'" class="poly-cursor poly-menu-item-delete tw-mr-1 text-muted pull-right"><i class="fas fa-trash"></i></span><span @click.stop="handleEdit(item)" v-if="item.is_custom=='true'" class="poly-cursor poly-menu-item-edit tw-mr-1 text-muted pull-right"><i class="fas fa-pencil"></i></span>
                                                </div>
...
echo '<script src="' . poly_utilities_common_helper::get_assets_minified('modules/poly_utilities/dist/assets/js/admin/custom_menu.js') . '"></script>';
